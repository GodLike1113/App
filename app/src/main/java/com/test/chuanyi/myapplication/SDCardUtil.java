...
/**
 * Author:  zengfeng
 * Time  :  2019/7/25 10:25
 * Des   :  https://blog.csdn.net/gaolei1201/article/details/51891608
 */
public class SDCardUtil {
    ...
    // public final static String DIR_UDISK_NAME = "USB存储设备";
...
//                }
                ...
                int minorIdx = (SDCardStat.Format.vfat == format || SDCardStat.Format.exfat == format || SDCardStat.Format.texfat == format) ? findVoldDevNodeMinorIndex(cols) : -100;
...
//                MainActivity.filterBuilder.append("path----:  "+path+"\n\n");
                ...
                if (android.os.Build.VERSION.SDK_INT >= 19) {
                    if (!SDCardUtil.sdCardCanWrite(path)) {
                        stat.canWrite = false;
...
//                    context.filterBuilder.append("path--other--:  " + path + "\n\n");
        ...
        for (int idx = 0, size = list.size(); idx < size; idx++) {
            if (idx == 0) {
                list.get(0).name = (size == 1) ? DIR_SINGLE_SDCARD_NAME : DIR_SDCARD_NAME;
            } else if (idx == 1) {
    ...
    private static boolean checkSDCardExistByRootPath(String path, ArrayList<SDCardStat> list) {
...
//            Log.d("gaolei", "lowerStr--------------"+lowerStr);
    ...
    public static String getShowSDPath(SDCardStat stat) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 19 && !stat.canWrite) {
    ...
    public static boolean compareData(ArrayList<SDCardStat> list, long capacity) {
        //排除内置或外置重复路径
        if (list.size() > 0) {
    ...
    public static class SDCardStat {
        ...
        public SDCardStat(String path, Format format, int voldMinorIdx, String excludePath) {
